 ...
 #               fd = open(os.path.join(curdir,field),"r")
  #              value = fd.read().replace("\n","").replace("\r","")
   #             fd.close()   
    ...
    t2 += "}</script>"
    t += t2
    if read_javascript_includes(): hgf_javascript_includes = read_javascript_includes()
    else: hgf_javascript_includes = ""  
    
    t += "</fieldset>%s" % hgf_javascript_includes 
    ...
    t = t + "<br /><br /><CENTER><INPUT type=\"button\" width=400 height=50 name=\"End\" value=\"Postpone\" id=\"postpone\" onClick=\"hgf_finish('mbipostpone');\"><INPUT type=\"button\" width=400 height=50 name=\"End1\" value=\"Finish & Release\" id=\"release\" onClick=\"hgf_finish('mbirelease');\"><INPUT type=\"button\" width=400 height=50 name=\"End2\" value=\"Delete Record\" id=\"deleted\" onClick=\"hgf_finish('mbidelete');\"></CENTER></H4>"
    #t += '<script>document.getElementById(\"continue\").click();</script>'
