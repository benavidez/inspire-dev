...
class Create_json_hgf_record:
	...
	def add_jsondict(self,fieldname):
		...
		for key in jsondict.keys():
			if key in self.data.keys(): return
			else: 
				if isinstance(jsondict[key],list): self.data[key] = jsondict[key] #value is already a list
					...
					if isinstance(eval(jsondict[key]),list): self.data[key] = eval(jsondict[key]) #value is a list in a string
					else: self.data[key] = [eval(jsondict[key])] #value is just a string
				else: return
				
	def add_one_field(self,marcfield,subfield,value):
		"""add field, no json structure"""
		if marcfield in self.data.keys(): 
			if subfield in self.data[marcfield][0].keys():self.data[marcfield].append({subfield:value}) 
	...
	def read_file(self,fieldname):
		...
		fr.close
		return text
		## in case we have repeatable fields without subfields return a list of values 
		#values = []
		#for line in lines:
...
def Create_hgf_record_json(parameters,curdir, form, user_info=None):
	...
	for fieldname in hgf_files:
		if fieldname == 'hgf_999C5': continue	
